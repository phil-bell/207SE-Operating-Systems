...
char cr_read_byte(cr_file* f){
  ...
  f->usedbuffer;
  f->bufferlength;
  printf("\n new byte \n");//when the byte is read "new byte" is printed to the screen
  if (f->usedbuffer == f->bufferlength)//the  buffer is refilled if the amount$
  {
    refill(f)
    printf("\n refilling \n"); // print "refilling" when the buffer is being reffiled
  }
  return f->buffer[f->usedbuffer++];//returns the buffer contents at the posit$
  // your code goes here
...
char cr_read_byte(cr_file* f){
  ...
  f->usedbuffer;
  f->bufferlength;
  f->total++; //the total amount of bytes used is incremented with every byte read
  printf("\n new byte \n");//when the byte is read "new byte" is printed to the screen
  if (f->usedbuffer == f->bufferlength)//the  buffer is refilled if the amount$
  {
    refill(f)
